...
def calcular_mrr(g_truth, descriptores_locales = None, 
    ...
    result_busc_desc = bus_desc.main(descriptores_locales, nombres, vectorizer, textos_consulta, n)
...
def main():
    ...
    # Verificar si la columna contiene g_truth de tipo cadena (strings)
        ...
        # Aplicar el método str.strip() para eliminar los espacios en blanco de los strings
            g_truth[columna] = g_truth[columna].str.strip()

    print(f'MRR para n-10 : {calcular_mrr(g_truth, descriptores_locales, nombres, vectorizer, n = 10)}')

    print(f'MRR para n-20 : {calcular_mrr(g_truth, descriptores_locales, nombres, vectorizer, n = 20)}')
